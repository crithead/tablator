...
# Then, roll once on each table.
if [ "$1" == "-h" ] || [ "$1" == "--help" ]; then
    readonly N="$(basename "$0")"
    cat <<EOD

    $N DATA_DIR - list tables and roll once on each
...
for json_file in $(find "$DATA_DIR" -name \*.json | sort); do
    ...
    printf '%s\n' ">>> Table: $table"
    "$PYTHON" "$TABLATOR" --data-dir "$DATA_DIR" --number "$N" "$table"
    [ $? -eq 0 ] || { echo ">>> Rolling on '$table' returned: $?"; exit 1; }
done
